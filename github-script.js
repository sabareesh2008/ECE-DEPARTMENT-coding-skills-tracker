...
const homeAiAnalystButton = document.getElementById("homeAiAnalystButton");
const aiAnalystButton = document.getElementById("aiAnalystButton");
const aiAnalystModal = document.getElementById("aiAnalystModal");
const aiChatForm = document.getElementById("aiChatForm");
const aiChatInput = document.getElementById("aiChatInput");
const aiChatMessages = document.getElementById("aiChatMessages");
const aiChatStatus = document.getElementById("aiChatStatus");
const aiDownloadArea = document.getElementById("aiDownloadArea");
...
function configured() {
  return Boolean(
    ...
    && !cfg.SUPABASE_ANON_KEY.includes("YOUR_PUBLISHABLE_KEY")
  );
}
...
function renderSectionChampionship() {
  ...
  if (!hasAnyStudents) {
    ...
    championBox.innerHTML = `
      ...
      <small>${champion.last30} contributions in 30 days</small>
    `;
  }
...
function updateCurrentChampionBadge() {
  ...
  scoreElement.textContent =
    `${champion.last30} contributions in last 30 days`;
}
  ...
  tableBody.innerHTML = students.map((student) => {
    ...
    const statusClass =
      status === "Success"
        ? "status-success"
        : (
            status === "GitHub Not Added" || status === "Pending"
              ? "status-pending"
              : "status-error"
          );
      ...
      <tr>
        ...
        <td>
          <button
            ...
            title="Open student GitHub progress profile"
          >
            ${escapeHTML(student["Student Name"] || "Student")} ↗
          </button>
        </td>

        <td><strong>${toNumber(student["Contributions 30 Days"])}</strong></td>
        <td>${toNumber(student["Contributions 14 Days"])}</td>
        <td>${toNumber(student["Contributions 7 Days"])}</td>
        <td>${toNumber(student["Commits 7 Days"])}</td>
        <td>${toNumber(student["Contributions Today"])}</td>
        <td><strong>${toNumber(student["Repositories Total"])}</strong></td>

        <td class="emh-cell">
          <span class="emh-e" title="30-Day Commits">${toNumber(student["Commits 30 Days"])}</span>
          <span class="emh-separator">/</span>
          <span class="emh-m" title="Repositories Created in 30 Days">${toNumber(student["Repositories 30 Days"])}</span>
          <span class="emh-separator">/</span>
          <span class="emh-h" title="Detected Deployed Projects">${toNumber(student["Detected Deployments"])}</span>
        </td>
...
async function loadRegisteredStudents() {
  ...
  // Seamlessly merge with allStudents from GitHubLiveData.csv / dashboard dataset
...
function renderManageStudents() {
  ...
  manageStudentsBody.innerHTML = students.map((student) => {
    ...
    const profileCell = username
      ...
      `
      : `<span class="student-status status-pending">Not Added</span>`;
...
async function saveProfile(event) {
  ...
  try {
    ...
    if (!targetId || targetId === registerNumber) {
    ...
    if (memStudent) {
      memStudent["Student Name"] = studentNameInput.value.trim();
      memStudent.student_name = studentNameInput.value.trim();
      memStudent["GitHub Username"] = githubUsername;
      memStudent.github_username = githubUsername;
      ...
      allStudents.push({
        id: result.data?.id || registerNumber,
...
function openDeleteModal(studentId) {
  ...
  deleteDescription.textContent =
    `Remove GitHub profile from ${student.student_name} (${student.section})? The student record and LeetCode profile will not be deleted.`;
...
function downloadCSV() {
  ...
  const fileName =
    selectedSection === "OVERALL"
      ? "ECE_GitHub_Overall_Leaderboard.csv"
      : `${selectedSection.replace(" ", "_")}_GitHub_Leaderboard.csv`;
...
async function loadProfileData() {
  ...
  const [historyResponse, activityResponse] = await Promise.all([
    fetch(`GitHubHistory.csv?time=${cacheBust}`, { cache: "no-store" }),
    fetch(`GitHubDailyActivity.csv?time=${cacheBust}`, { cache: "no-store" })
  ]);
...
function renderDifficultyChart(student) {
  const values = [
    ["30D Commits", profileNumber(student["Commits 30 Days"]), "easy"],
    ["30D Repos", profileNumber(student["Repositories 30 Days"]), "medium"],
    ["Deployed", profileNumber(student["Detected Deployments"]), "hard"]
  ];
      ...
      return `
        <div class="difficulty-row">
          ...
          <div class="difficulty-track">
            <span
              class="difficulty-fill difficulty-${tone}"
              style="width:${width}%"
            ></span>
          </div>
        </div>
...
function renderProgressChart(registerNumber, student) {
  const rows = historyRows
    ...
    .map((row) => ({
      date: row.Date,
      total: profileNumber(row["Contributions 30 Days"])
    }))
    .filter((row) => row.date)
  ...
  if (updatedDate) {
    byDate.set(
      updatedDate,
      profileNumber(student["Contributions 30 Days"])
    );
  }
  ...
  if (!points.length) {
    ...
    container.innerHTML = `
      <div class="profile-empty-chart">
        GitHub contribution history will appear after tracker snapshots are collected.
      </div>
    `;
    return;
  }
  ...
  if (points.length === 1) {
    container.innerHTML = `
      <div class="single-progress-point">
        ...
        <small>30-day contributions</small>
      </div>
    `;
    return;
  }
  ...
  const circles = points.map((point, index) => `
    ...
    >
      <title>${formatProfileDate(point.date)}: ${point.total} contributions</title>
    </circle>
  `).join("");
  ...
  container.innerHTML = `
    <svg
      ...
      role="img"
      aria-label="30-day GitHub contributions trend"
    >
      ${grid}
...
function renderDailyActivity(registerNumber) {
  const rows = dailyActivityRows
    ...
    .map((row) => ({
      date: row.Date,
      solved: profileNumber(row["Contributions That Day"])
    }))
    .filter((row) => row.date)
    .sort((a, b) => String(a.date).localeCompare(String(b.date)))
    .slice(-14);
  ...
  if (!rows.length) {
    container.innerHTML = `
      <div class="profile-empty-chart">
        Daily GitHub contributions will appear after tracker snapshots are collected.
      </div>
    `;
    return;
  }
...
async function openStudentProfile(registerNumber) {
  ...
  document.getElementById("profileUsername").textContent =
    `@${student["GitHub Username"] || "username"}`;
  ...
  document.getElementById("profileTotalSolved").textContent =
    student["Repositories Total"] || "0";
  document.getElementById("profile30Days").textContent =
    student["Contributions 30 Days"] || "0";
  document.getElementById("profile14Days").textContent =
    student["Contributions 14 Days"] || "0";
  document.getElementById("profile7Days").textContent =
    student["Contributions 7 Days"] || "0";
  document.getElementById("profile7DaySubmissions").textContent =
    student["Commits 7 Days"] || "0";
  document.getElementById("profileToday").textContent =
    student["Contributions Today"] || "0";

  document.getElementById("profileLastProblem").textContent =
    student["Latest Repository"] || "–";
  ...
  leetCodeLink.href =
    student["GitHub Link"]
    || `https://github.com/${encodeURIComponent(student["GitHub Username"] || "")}`;
...
async function loadFacultyData() {
  ...
  const active = document.getElementById("facultyActiveToday");
  const seven = document.getElementById("facultySolved7Days");

  if (count) count.textContent = "0";
  if (active) active.textContent = "0";
  if (seven) seven.textContent = "0";
}
...
function formatAiAnswer(text) {
  const escaped = escapeAiText(text);
  ...
  return escaped
    ...
    .replace(/^#\s+(.+)$/gm, "<h3>$1</h3>")
    .replace(/^[-•]\s+(.+)$/gm, "<div class=\"ai-answer-bullet\">• $1</div>")
    .replace(/\n/g, "<br>");
}
...
function appendAiMessage(role, text, options = {}) {
  ...
  title.textContent = role === "user" ? "You" : "Performance Analyst";
  content.appendChild(title);

  const body = document.createElement("p");

  if (options.thinking) {
    body.innerHTML = `<span class="ai-thinking"><span>Analyzing live data</span><i></i><i></i><i></i></span>`;
  } else {
    body.innerHTML = formatAiAnswer(text);
  }
...
function setAiBusy(busy) {
  aiChatBusy = busy;
  sendAiChatButton.disabled = busy;
  aiChatInput.disabled = busy;
  aiChatStatus.textContent = busy
...
function clearAiChat() {
  aiChatHistory = [];
  aiDownloadArea.hidden = true;
  aiDownloadArea.innerHTML = "";
  ...
  aiChatMessages.innerHTML = `
    <article class="ai-message ai-message-assistant">
      ...
      <div class="ai-message-content">
        ...
        <p>Conversation cleared. Ask me about students, sections, faculty, challenges, coding tests or request a downloadable report.</p>
      </div>
    </article>
  `;
}
...
function showAiDownload(file) {
  if (!file?.data_base64 || !file?.filename) {
    aiDownloadArea.hidden = true;
    aiDownloadArea.innerHTML = "";
    return;
  }

  aiDownloadArea.hidden = false;
  aiDownloadArea.innerHTML = `
  ...
  document.getElementById("downloadAiGeneratedFile")
...
async function askAiPerformanceAnalyst(message) {
  ...
  try {
    ...
    const answer = data.answer || "Analysis completed.";
...
usernameInput.addEventListener("input", () => {
  generatedLink.textContent =
    `https://github.com/${usernameInput.value.trim() || "username"}`;
});
...
homeAiAnalystButton?.addEventListener("click", openAiAnalyst);
aiAnalystButton?.addEventListener("click", openAiAnalyst);

document.getElementById("closeAiAnalystButton")
  ?.addEventListener("click", closeAiAnalyst);

document.querySelector("[data-close-ai-analyst]")
  ?.addEventListener("click", closeAiAnalyst);

document.getElementById("clearAiChatButton")
  ?.addEventListener("click", clearAiChat);

document.querySelectorAll("[data-ai-prompt]").forEach((button) => {
  button.addEventListener("click", () => {
    aiChatInput.value = button.dataset.aiPrompt || "";
    aiChatInput.focus();
  });
});
...
aiChatForm?.addEventListener("submit", async (event) => {
  ...
  if (!message) return;
  aiChatInput.value = "";
  await askAiPerformanceAnalyst(message);
});
...
// ACTIONS MENU MODAL
// ============================================================
const openAdminActionsMenuButton =
  document.getElementById("openAdminActionsMenu");

const adminActionsModal =
  document.getElementById("adminActionsModal");

const closeAdminActionsMenuButton =
  document.getElementById("closeAdminActionsMenu");

function openAdminActionsMenu() {
  if (!adminActionsModal) return;

  adminActionsModal.hidden = false;
  document.body.classList.add("modal-open");

  window.setTimeout(() => {
    closeAdminActionsMenuButton?.focus();
  }, 0);
}
...
function closeAdminActionsMenu() {
  ...
  adminActionsModal.hidden = true;
  document.body.classList.remove("modal-open");

  openAdminActionsMenuButton?.focus();
}

openAdminActionsMenuButton?.addEventListener(
  "click",
  openAdminActionsMenu
);

closeAdminActionsMenuButton?.addEventListener(
  "click",
  closeAdminActionsMenu
);

document
  .querySelector("[data-close-admin-actions]")
  ?.addEventListener("click", closeAdminActionsMenu);

adminActionsModal?.addEventListener("click", (event) => {
  const actionButton = event.target.closest(
    ".admin-action-tile"
  );

  if (actionButton) {
    // Existing action listener executes normally first.
    window.setTimeout(() => {
      if (!adminActionsModal.hidden) {
        closeAdminActionsMenu();
      }
    }, 0);
  }
