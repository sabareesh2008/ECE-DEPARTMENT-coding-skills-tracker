#!/usr/bin/env python3
"""
ECE LeetCode Platform - Automated Daily / Weekly Reports

Data sources:
- LiveData.csv       -> LeetCode tracker metrics
- LeetCode public profile feed -> exact reporting-window activity
...
WhatsApp:
- WhatsApp Cloud API
- Section-wise faculty routing for ECE A-F and HOD Overall.
- PDF report attached to WhatsApp template message.
...
Environment variables:
  ...
  WHATSAPP_ECE_A_NUMBER
  WHATSAPP_ECE_B_NUMBER
  WHATSAPP_ECE_C_NUMBER
  WHATSAPP_ECE_D_NUMBER
  WHATSAPP_ECE_E_NUMBER
  WHATSAPP_ECE_F_NUMBER
  WHATSAPP_HOD_NUMBER
...
# ============================================================
# REPORT DELIVERY SWITCHES
# ============================================================

# Email can be turned ON/OFF from GitHub Actions.
SEND_EMAIL = env("SEND_EMAIL", "true").lower() == "true"

# WhatsApp can be turned ON/OFF from GitHub Actions.
SEND_WHATSAPP = env("SEND_WHATSAPP", "true").lower() == "true"
...
def load_config(
    ...
    whatsapp_numbers = {
        section: env(
            f"WHATSAPP_{section.replace(' ', '_')}_NUMBER"
        )
        for section in SECTIONS
    }

    whatsapp_hod_number = env(
        "WHATSAPP_HOD_NUMBER"
    )
    ...
    whatsapp = WhatsAppConfig(
        ...
        daily_template=env(
            "WHATSAPP_DAILY_TEMPLATE",
            "codemetrix_daily_report",
        ),
        weekly_template=env(
            "WHATSAPP_WEEKLY_TEMPLATE",
            "codemetrix_weekly_report",
        ),
    ...
    if require_email:
        ...
        if (
            not config.hod_recipients
            and not any(
                config.section_recipients.values()
            )
        ):
            missing.append(
                "REPORT_HOD_EMAILS or at least one "
                "REPORT_ECE_*_EMAILS secret"
            )
    ...
    if require_whatsapp:
        ...
        if (
            not config.whatsapp.hod_number
            and not any(
                config.whatsapp.section_numbers.values()
            )
        ):
            whatsapp_missing.append(
                "WHATSAPP_HOD_NUMBER or at least one "
                "WHATSAPP_ECE_*_NUMBER"
            )
...
def parse_datetime(
    ...
    """Parse Supabase timestamps and return IST-aware datetime."""
...
def load_live_data() -> pd.DataFrame:
    ...
    if missing:
        ...
        raise RuntimeError(
            "LiveData.csv is missing required column(s): "
            + ", ".join(missing)
        )
...
def _normalize_report_submissions(
...
def fetch_report_activity(
    ...
    """Fetch report data without private submission calendar."""
    ...
    if not username:
        ...
        return {
            ...
            "solved": 0,
            "solved_coverage": "ERROR",
            "recent_attempts": 0,
            "attempt_coverage": "ERROR",
            "current_total_submissions": None,
        }

    body = {
        "operationName": "reportActivity",

        "query": REPORT_ACTIVITY_QUERY,
    ...
    for attempt in range(1, 4):
        ...
        try:
            ...
            ac_rows = _normalize_report_submissions(
                data.get(
                    "recentAcSubmissionList",
                    [],
                )
            ...
            attempt_rows = _normalize_report_submissions(
                data.get(
                    "recentSubmissionList",
                    [],
                )
            ...
            solved = _count_solved_between(
                ac_rows,
                start_time,
                end_time,
            )

            attempts = _count_attempts_between(
                attempt_rows,
                start_time,
                end_time,
            )
...
def supabase_report_snapshot_get(
    config: Config,
    boundary: datetime,
...
def supabase_report_snapshot_upsert(
    config: Config,
    live: pd.DataFrame,
    boundary: datetime,
) -> None:
...
def format_window(
    ...
    return (
        ...
        f"to "
        f"{end.strftime('%d %b %Y %I:%M %p')} IST"
    )


def count_distinct_accepted_between(
    recent_submissions,
    start_time: datetime,
    end_time: datetime,
) -> int:
...
def _window_activity_for_username(
    username: str,
    start_time: datetime,
    end_time: datetime,
) -> dict[str, Any]:
    ...
    profile = leetcode_tracker.fetch_leetcode(
        username
    )
    ...
    solved = count_distinct_accepted_between(
        recent,
        start_time,
        end_time,
    )
    ...
    full = (
        leetcode_tracker.accepted_feed_covers_window(
            recent,
            profile.get(
...
def refresh_report_window_activity(
    live: pd.DataFrame,
    start_time: datetime,
    end_time: datetime,
    config: Config,
    ...
    if frame.empty:
        ...
        for c in [
            "Report Window Solved",
            "Report Window Coverage",
            "Report Window Status",
            "Report Window Submissions",
            "Report Submission Coverage",
            "_Current Total Submissions",
        ]:
    ...
    baseline = supabase_report_snapshot_get(
        config,
        start_time,
        registers,
    )
...
def build_daily_report(
    ...
    active, inactive, unknown = (
        report_window_masks(live)
    )

    top = report_window_students(
        live,
        10,
        False,
    )

    zeros = inactive_students(
        live
    )

    sections = section_report_summary(
        live
    )
    ...
    top_rows = [
        [
            ...
            x["value"],
            _display_submission(
                x["window_submissions"],
                x["submission_coverage"],
            ),
            x["week"],
            x["fortnight"],
            x["month"],
    ...
    <div class="card">
      ...
      <div class="kpis">
        ...
        <div class="kpi">
          ...
          <strong>{int(active.sum())}</strong>
        </div>

        <div class="kpi">
          <span>0 Solved / 0 Submission</span>
          <strong>{int(inactive.sum())}</strong>
        </div>
    ...
    <div class="card">
      ...
      <h2>
        0 Solved Today Students
      </h2>

      <p class="muted">
        Only students with 0 solved AND 0 submissions
        in the complete 07:00 AM to 07:00 AM window
        are listed. Students who submitted code today
        are excluded.
      </p>
...
def build_weekly_report(
    ...
    <div class="card">
      ...
      <h2>
        0 Solved This Week Students
      </h2>

      <p class="muted">
        Only students with 0 solved AND 0 submissions
        in the complete weekly report window are listed.
      </p>

      {table_html(headers, rows(zeros))}
    ...
    <div class="card">
      ...
      <h2>
        Bottom 10 Students - This Week
      </h2>

      <p class="muted">
        Completely inactive 0/0 students are shown separately
        above and are excluded here.
      </p>

      {table_html(headers, rows(bottom))}
...
def generate_daily_excel(
    ...
    path = (
        REPORT_DIR
        / f"{scope_slug(scope_label)}"
          f"_Daily_Report_{report_date}.xlsx"
    )
    ...
    with pd.ExcelWriter(
        ...
        _write_dataframe_sheet(
            ...
            "0 Solved Today",
            frame(zeros),
            "0 Solved Today - Also 0 Today Submissions",
        )
...
def generate_weekly_excel(
    ...
    path = (
        REPORT_DIR
        / f"{scope_slug(scope_label)}"
          f"_Weekly_Report_{start_date}"
          f"_to_{end_date}.xlsx"
    )
    ...
    with pd.ExcelWriter(
        ...
        _write_dataframe_sheet(
            ...
            "0 Solved Week",
            frame(zeros),
            "0 Solved This Week - Also 0 Weekly Submissions",
        )
...
def generate_daily_pdf(
    ...
    path = (
        REPORT_DIR
        / f"{scope_slug(scope_label)}"
          f"_Daily_Report_{report_date}.pdf"
    )
    ...
    story = [
        ...
        Paragraph(
            "Only 0 solved + 0 submission students are listed.",
            styles["subtitle"],
        ),
...
def generate_weekly_pdf(
    ...
    story = [
        ...
        Paragraph(
            "Only 0 solved + 0 weekly submission students are listed.",
            styles["subtitle"],
        ),
        ...
        Paragraph(
            "Completely inactive 0/0 students are excluded here.",
            styles["subtitle"],
        ),
...
def send_gmail_email(
    ...
    if not recipients:
        ...
        raise RuntimeError(
            "No report recipients configured for this report."
        )
...
def build_report(
    ...
    if mode == "daily":
        ...
        live = refresh_report_window_activity(
            live,
            report_start,
            report_end,
            config,
        ...
        if not offline:
            ...
            supabase_report_snapshot_upsert(
                config,
                live,
                report_end,
            )
...
def write_html_copy(
    ...
    output = (
        ...
        / (
            f"latest_{mode}_"
            f"{scope_slug(route_label).lower()}"
            "_report.html"
        )
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip LeetCode refresh and use "
            "LiveData only for local preview."
        ),
    )
    ...
    for (
        ...
        print(
            f"Building {args.mode} report "
            f"for {route_label}"
        )

        subject, html_body, attachment_paths, report_live = (
            build_report(
                args.mode,
                config,
        ...
        if args.dry_run:
            ...
            print(
                f"DRY RUN: {route_label} "
                "email and WhatsApp not sent."
            )
        ...
        whatsapp_number = (
            ...
            else config.whatsapp.section_numbers.get(
                route_label,
                "",
            )
        )
        ...
        active, inactive, _unknown = (
            report_window_masks(
                report_live
            )
        )

        inactive_rows = inactive_students(
            report_live
        )
        ...
        try:
            ...
            print(
                f"{route_label}: "
                "WhatsApp report sent to "
                f"recipient {whatsapp_number}. "
