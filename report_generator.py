...
def parse_recipients(raw: str) -> list[str]:
    """Parse comma/semicolon/newline-separated emails and remove duplicates safely."""
    ...
    for item in normalized.split(","):
        address = item.strip()
        if not address or address in {"***", "none", "null", "undefined", "N/A"}:
            continue
        ...
        if "@" not in address or address.startswith("@") or address.endswith("@"):
            print(f"WARNING: Skipping invalid email recipient format: '{address}'", file=sys.stderr)
            continue
    ...
    for group in chunks(recipients, MAX_RECIPIENTS_PER_EMAIL):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Ensure GMAIL_ADDRESS is correct and GMAIL_APP_PASSWORD is a valid 16-character Google App Password "
...
def main() -> int:
    ...
    if scope_filter:
        selected_routes = [
            (
                scope_filter,
                None if scope_filter == "OVERALL" else scope_filter,
                (
                    config.hod_recipients
                    if scope_filter == "OVERALL"
                    else config.section_recipients.get(scope_filter, [])
                ),
            )
        ]
    else:
    ...
    for route_label, section, recipients in selected_routes:
        ...
        try:
            ...
            err_msg = f"{route_label} ({', '.join(recipients)}): {exc}"
            print(f"\n[!] EMAIL ERROR for {err_msg}\n", file=sys.stderr)
            email_errors.append(err_msg)
    ...
    if args.dry_run:
        ...
        print("=" * 72)
        print(
            f"SECTION-WISE EMAIL COMPLETE: {sent_reports} report(s) sent, "
            f"{sent_recipients} recipient(s)."
        )

        if email_errors:
            print("\n" + "=" * 72, file=sys.stderr)
            print("EMAIL DISPATCH WARNINGS / ERRORS:", file=sys.stderr)
            for err in email_errors:
                print(f"  * {err}", file=sys.stderr)
            print("=" * 72 + "\n", file=sys.stderr)

            if sent_reports == 0 and not args.dry_run:
                raise RuntimeError(
                    f"All configured email dispatches failed ({len(email_errors)} error(s)). "
